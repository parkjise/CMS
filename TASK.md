...
- **작업 내용:**
  - [ ] `gallery_items` 테이블 추가 마이그레이션
  - [ ] `POST /api/v1/upload/gallery` — 다중 파일 업로드 (최대 10개 동시)
  - [ ] `GET /api/v1/gallery/{section_id}` — 갤러리 아이템 목록
  - [ ] `DELETE /api/v1/gallery/{item_id}` — 갤러리 아이템 삭제 + 파일 삭제
  - [ ] `PATCH /api/v1/gallery/order` — 갤러리 이미지 순서 변경
...
- **참조:** `기획서 섹션 9 (Celery)`
- **작업 내용:**
  - [ ] `app/workers/celery_app.py` — Celery 앱 초기화 (Redis 브로커)
  - [ ] `app/workers/image.py`
...
- **작업 내용:**
  - [ ] `app/workers/notification.py`
    - `send_inquiry_alert` 태스크 — 문의 접수 알림 비동기 발송
    - 재시도 3회, 지수 백오프
    - 발송 완료 후 `inquiries.is_notified = True` 업데이트
  - [ ] 문의 접수 엔드포인트에서 태스크 트리거 연동
  - [ ] Redis Pub/Sub publish 연동 (SSE 실시간 알림)
...
- **작업 내용:**
  ...
  - [ ] 통계 카드 컴포넌트 (오늘 방문자, 이번주 방문자, 신규 문의, 미처리 문의)
  - [ ] Recharts Line Chart (7일 방문자 추이)
  - [ ] 미확인 문의 목록 (클릭 시 AD-04로 이동)
  - [ ] `useSSENotifications` 훅 연동 (실시간 카운터 업데이트)
  - [ ] 스켈레톤 로딩 UI
...
- **작업 내용:**
  ...
  - [ ] 섹션 목록 + 활성화 토글
  - [ ] `@dnd-kit/react` 드래그앤드롭 (--legacy-peer-deps)
  - [ ] 낙관적 업데이트 (순서 변경 즉시 UI 반영, API 실패 시 롤백)
  - [ ] 섹션 유형별 아이콘 표시
  - [ ] 탭 구조: [섹션 관리] [공통 설정]
...
- **작업 내용:**
  ...
  - [ ] 글자 수 실시간 카운터 (초과 시 빨간색)
  - [ ] ImageUpload 컴포넌트 연동
  - [ ] [기본 설정] [모바일 설정] 탭 구조
  - [ ] 변경 시 자동 저장 방지 + [변경사항 저장] 버튼
...
- **참조:** `기획서 섹션 4 (AD-03)`
- **작업 내용:**
  - [ ] `apps/admin/src/pages/SnsPage.tsx`
  - [ ] SNS 채널 목록 (인스타그램, 네이버 블로그, 카카오, 유튜브, 페이스북)
  - [ ] URL 입력 + 연결 테스트 버튼 (API 호출 후 결과 표시)
  - [ ] 활성화 토글 (URL 없으면 토글 비활성화)
  - [ ] 노출 위치 체크박스 (푸터, 플로팅)
  - [ ] 알림 채널 설정 (카카오/SMS/이메일 + 전화번호 인증)
  - [ ] 이번 달 발송 현황 프로그레스 바
