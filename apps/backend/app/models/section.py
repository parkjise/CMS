...
class Section(Base, TimestampMixin, SoftDeleteMixin):
    ...
    tenant: Mapped["Tenant"] = relationship("Tenant", back_populates="sections", lazy="noload")
    settings: Mapped[list["SectionSetting"]] = relationship(
        "SectionSetting", back_populates="section", cascade="all, delete-orphan", lazy="noload"
    )
...
class SectionSetting(Base, TimestampMixin):
    ...
    section: Mapped["Section"] = relationship("Section", back_populates="settings", lazy="noload")
